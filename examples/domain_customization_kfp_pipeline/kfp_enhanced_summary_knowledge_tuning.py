...
def create_seed_data_component(
    ...
    def create_seed_data_from_quality_benchmark(seed_data_path: str):
        ...
        quality_corpus = load_dataset(
            ...
            {'raw': 'document', 'uid': 'document_outline'}
        )
...
def merge_all_outputs_component(
    ...
    extractive.to_json(os.path.join(merged_output.path, 'extractive_summary', 'gen.jsonl'), orient='records', lines=True)
    detailed.to_json(os.path.join(merged_output.path, 'detailed_summary', 'gen.jsonl'), orient='records', lines=True)
    key_facts.to_json(os.path.join(merged_output.path, 'key_facts_to_qa', 'gen.jsonl'), orient='records', lines=True)
    doc_qa.to_json(os.path.join(merged_output.path, 'document_based_qa', 'gen.jsonl'), orient='records', lines=True)
...
def knowledge_generation_pipeline(
    ...
    merge_task = merge_all_outputs_component(
...
if __name__ == "__main__":
    ...
    print("   kubectl create secret generic sdg-pipeline-config --from-env-file=.env -n kubeflow")
